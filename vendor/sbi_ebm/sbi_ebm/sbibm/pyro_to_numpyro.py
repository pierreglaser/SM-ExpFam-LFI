...
@overload
def convert_dist(d: Union[pyro_distributions.Distribution, tdist.Distribution], implementation: Literal["pyro"]) -> Union[pyro_distributions.Distribution, tdist.Distribution]: ...

@overload
def convert_dist(d: Union[pyro_distributions.Distribution, tdist.Distribution], implementation: Literal["numpyro"]) -> np_distributions.Distribution: ...


def convert_dist(d: Union[pyro_distributions.Distribution, tdist.Distribution], implementation: Literal["pyro", "numpyro"]) -> Union[np_distributions.Distribution, pyro_distributions.Distribution, tdist.Distribution]:
...
_register_transforms_converter(_convert_independent_transform, tdist.transforms.IndependentTransform)
...
def _convert_compose_transforms(
    ...
    if implementation == "numpyro":
        if len(t.parts) == 0:
            ...
            return np_transforms.ComposeTransform([convert_transform(part, implementation) for part in t.parts])

    else:
        raise ValueError

_register_transforms_converter(_convert_compose_transforms, tdist.transforms.ComposeTransform)
