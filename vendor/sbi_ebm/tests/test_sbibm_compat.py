...
from sbi_ebm.sbibm.sbi_ebm import seed_everything
...
def test_distribution_conversion(task_name: str):
    ...
    wrapped_pyro_prior  = wrap_prior_dist(pyro_prior, get_task(task_name)._get_transforms()["parameters"])
    ...
    wrapped_converted_log_ls = converted_wrapped_prior.log_prob(jnp.array(wrapped_samples))

    assert jnp.allclose(wrapped_pyro_log_ls, wrapped_converted_log_ls, rtol=1e-5, atol=1e-5)
